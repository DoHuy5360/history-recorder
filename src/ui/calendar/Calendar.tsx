...
import { setDataTasks, setStartDate, setEndDate, setCurrentMonth, currentActualMonth } from "../redux/reducers/_calendar";
...
function View() {
	...
	const { isShowAddEventForm, dayAddedEvent } = useAppSelector((state) => state.createEventFormReducer);
	...
	return (
		<div className="p-3">
			...
			<div className="border-slate-100 border-[1px] select-none">
				<div className="p-2 text-center">{currentYear}</div>
				<div className="grid grid-cols-12">
					{Array.from({ length: 12 }, (_, index) => {
						const monthAsNumber = index + 1;
						const isCurrentMonth = currentActualMonth === monthAsNumber;
						return (
							<div
								key={index}
								onClick={(e) => {
									dispatch(setCurrentMonth(monthAsNumber as unknown as number));
								}}
								className={`${currentMonth == monthAsNumber && "bg-slate-50"} hover:bg-slate-100 cursor-pointer p-2 text-center`}>
				...
				<div className="grid grid-cols-7 gap-[1px] bg-slate-100">
					...
					{dataTasks &&
						dataTasks.days.map((day: Day<string>, index) => {
							...
							const isToday = day.day === currentDay && currentMonth === currentActualMonth;
							...
							const isDaySelected = (isShowAddTaskForm || isShowAddEventForm) && (dayAddedTask === day.day || dayAddedEvent === day.day);
							return (
								!Number.isNaN(dayInWeek) && (
									<div
										...
										${isDaySelected && "outline-2 outline-dashed outline-purple-1 z-10"}
								${
									isDaysBetweenStartAndEnd ? "bg-yellow-100" : day.day !== startDate && day.day !== endDate && "bg-white"
								} h-full select-non outline outline-0 outline-slate-400 hover:outline-1 cursor-pointer transition-colors ease-in-out duration-100
								`}
										style={{
